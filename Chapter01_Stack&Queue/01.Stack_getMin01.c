...
/*压栈
    ...
    如果不为空，则比较newNum与stackMin栈顶元素哪一个更小；
        如果newNum更小或两者相等，则将newNum压入；
        如果stackMin栈顶元素更小，则重复压入stackMin栈顶元素
    stackData与stackMin元素数量始终相等
 */
void SpecialPush(Stack stackData, Stack stackMin, int newNum) {
    if (IsEmpty(stackMin) == 1) {
        ...
        Push(stackMin, getMin(stackMin));
    }
    Push(stackData, newNum);
}

/*弹栈
    弹出并返回stackData数据，同时弹出stackMin数据
 */
